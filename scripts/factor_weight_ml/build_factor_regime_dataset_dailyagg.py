...
# ---------------------------------------------------------
# Feature / filter alignment helpers
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Monthly forward returns for next rebalance period
# ---------------------------------------------------------
...
def _build_benchmark_context(
    ...
    cols = [
        ...
        "benchmark_mdd_60d",
        "benchmark_mdd_120d",
        "dailyagg__benchmark__ret_20d",
        "dailyagg__benchmark__ret_60d",
        "dailyagg__benchmark__vol_20d",
        "dailyagg__benchmark__vol_60d",
        "dailyagg__benchmark__mdd_60d",
        "dailyagg__benchmark__mdd_120d",
    ]

    px = raw_px.loc[raw_px["ticker"] == str(benchmark_ticker).zfill(6)].copy()
    if len(px) == 0:
        return pd.DataFrame(columns=cols)

    left = pd.DataFrame({"rebalance_month": pd.to_datetime(rebalance_months)}).sort_values("rebalance_month")
    right = px[[
        "date", "ret_20d", "ret_60d", "vol_20d", "vol_60d", "mdd_60d", "mdd_120d"
    ]].sort_values("date")
    ...
    merged["dailyagg__benchmark__ret_20d"] = merged["ret_20d"]
    merged["dailyagg__benchmark__ret_60d"] = merged["ret_60d"]
    merged["dailyagg__benchmark__vol_20d"] = merged["vol_20d"]
    merged["dailyagg__benchmark__vol_60d"] = merged["vol_60d"]
    merged["dailyagg__benchmark__mdd_60d"] = merged["mdd_60d"]
    merged["dailyagg__benchmark__mdd_120d"] = merged["mdd_120d"]

    merged = merged.rename(columns={
        "ret_20d": "benchmark_ret_20d",
        "ret_60d": "benchmark_ret_60d",
        "vol_20d": "benchmark_vol_20d",
        "vol_60d": "benchmark_vol_60d",
        "mdd_60d": "benchmark_mdd_60d",
        "mdd_120d": "benchmark_mdd_120d",
    })
    return merged


def _add_dailyagg_row_features(
    row: dict[str, Any],
    scored: pd.DataFrame,
    px: pd.DataFrame,
    rm: pd.Timestamp,
    ...
    if len(snap) == 0:
        for c in [
            ...
            "dailyagg__universe__ret_20d_std",
            "dailyagg__universe__ret_60d_mean",
            "dailyagg__universe__ret_60d_std",
            "dailyagg__universe__vol_20d_mean",
            "dailyagg__universe__tv_mean_20d_mean",
            "dailyagg__universe__tv_mean_60d_mean",
        ]:
            row[c] = np.nan
        for bucket in active_buckets.keys():
            row[f"dailyagg__{bucket}__ret_20d_mean"] = np.nan
            row[f"dailyagg__{bucket}__ret_60d_mean"] = np.nan
            row[f"dailyagg__{bucket}__tv_mean_20d_mean"] = np.nan
        return row

    snap = snap.merge(
        scored[["ticker"] + [f"{b}__score" for b in active_buckets.keys()]],
        on="ticker",
        how="left",
    )

    # universe-level daily aggregates
    row["dailyagg__universe__ret_20d_mean"] = _safe_mean(snap["ret_20d"])
    row["dailyagg__universe__ret_20d_std"] = _safe_std(snap["ret_20d"])
    row["dailyagg__universe__ret_60d_mean"] = _safe_mean(snap["ret_60d"])
    row["dailyagg__universe__ret_60d_std"] = _safe_std(snap["ret_60d"])
    ...
    row["dailyagg__universe__tv_mean_60d_mean"] = _safe_mean(snap["tv_mean_60d"])

    # bucket-level top-N daily aggregates
    for bucket in active_buckets.keys():
        score_col = f"{bucket}__score"
        part = snap.dropna(subset=[score_col]).copy()
        if len(part) == 0:
            row[f"dailyagg__{bucket}__ret_20d_mean"] = np.nan
            row[f"dailyagg__{bucket}__ret_60d_mean"] = np.nan
            row[f"dailyagg__{bucket}__tv_mean_20d_mean"] = np.nan
        ...
        row[f"dailyagg__{bucket}__ret_60d_mean"] = _safe_mean(part["ret_60d"])
        row[f"dailyagg__{bucket}__tv_mean_20d_mean"] = _safe_mean(part["tv_mean_20d"])
...
def main() -> None:
    ...
    ap.add_argument("--top_k", type=int, default=10, help="top-k used for baseline and bucket forward return calculation")
    ap.add_argument("--bucket_topn", type=int, default=20, help="top-n within each bucket for daily aggregate feature calculation")
    ...
    if p_raw is None:
        raise FileNotFoundError("raw daily price file not found for dailyagg builder")
    ...
    if not active_buckets:
        raise RuntimeError(
            "No active factor buckets found. Check strategy weights vs features_live columns."
        )
    ...
    for i in range(len(rb_months) - 1):
        ...
        row: dict[str, Any] = {
            ...
            "bucket_topn": int(args.bucket_topn),
        ...
        # NEW: daily aggregated context
        row = _add_dailyagg_row_features(
            row=row,
            scored=scored,
            px=raw_px,
            rm=rm,
    ...
    # attach benchmark daily context
    if len(bm_ctx) > 0:
    ...
    if args.out_path:
        ...
        out_path = Path(
            rf"data/factor_weight_ml/datasets/factor_regime_dataset_dailyagg__asof={args.asof}"
            rf"__metric={args.metric}__strat={args.strategy}__featv={args.feat_v}"
            rf"__retv={args.ret_v}__topk={args.top_k}__topn={args.bucket_topn}__v={args.out_v}.parquet"
        )
