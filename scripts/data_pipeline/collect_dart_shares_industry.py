# scripts/data_pipeline/collect_dart_shares_industry.py
...
def main() -> None:
    ...
    for _, r in df.iterrows():
        ...
        # (B) DART company.json industry code (diagnostic only; do NOT treat as final KRX 6-digit code)
    ...
    # Canonical final industry attachment: 6-digit KRX-style code + name by ticker
    if args.with_industry:
        ref = load_industry_reference(args.industry_ref_csv)
        if len(ref) > 0:
            out = out.merge(ref, on="ticker", how="left")
            out["industry_source"] = out["industry_code"].notna().map({True: "reference_by_ticker", False: "missing_reference"})
        else:
            out["industry_code"] = pd.NA
            out["industry_name"] = pd.NA
            out["industry_source"] = "reference_missing"

        # strict 6-digit validation
        if "industry_code" in out.columns:
            out["industry_code"] = out["industry_code"].astype("string").str.extract(r"(\d+)")[0]
            ...
            if bad.any():
                ...
                out.loc[bad, "industry_name"] = pd.NA
                out.loc[bad, "industry_source"] = "invalid_reference_code"
    ...
    print(
        ...
        f"shares_null_ratio={out['shares'].isna().mean():.3f} "
        f"dart_induty_null_ratio={out['dart_induty_code'].isna().mean():.3f} "
