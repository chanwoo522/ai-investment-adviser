...
NAME_CANDIDATES = ["종목명", "종목 명", "한글명", "종목"]
SHARES_CANDIDATES = ["보유잔고", "보유수량", "수량", "잔고수량", "잔고"]

# 매핑 파일 컬럼 후보
MASTER_NAME_CANDIDATES = ["종목명", "한글종목명", "name", "name_final"]
...
def _detect_header_start(raw: pd.DataFrame) -> int:
    ...
    raise ValueError("보유내역 헤더 시작행을 찾지 못했습니다. 원본 파일 형식을 확인하세요.")
...
def _extract_holdings(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # 숫자만 있는 행 제거 (평가금액/매입금액 등이 종목명처럼 잡힌 경우)
    extracted = extracted[
...
def _auto_find_master(asof: str | None) -> Path:
    # 기본 fallback. 지금 오빠는 --master 직접 주는 걸 추천.
...
def main() -> None:
    ...
    ap.add_argument("--master", default=None, help="종목명-티커 매핑 파일 경로 (.xlsx/.csv/.parquet)")
    ...
    ap.add_argument("--allow_missing_ticker", action="store_true", help="ticker 누락 종목이 있어도 저장")
    args = ap.parse_args()
